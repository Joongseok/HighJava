...
/*
	...
	다양한 언어, 방언 및 음성으로 텍스트를 자연스러운 발음으로 합성하는
	API를 제공한다.
	이 서비스는 각 언어에 대해 남성 또는 여성, 또는 둘다 지원한다.
*/
public class TestToSpeechTest {
	
	// IBM에서 회원 등록후 해당 서비스를 사용할 수 있는 라이센스 ID와 PassWord를
	// 제공받아서 사용한다.
	...
	/*
		서비스 헤더 설정 메서드
		- 왓슨은 기본적으로 서비스 사용에 대한 로그를 남겨 서비스를
		  개선하는데 사용하고 있다.
		- 만약 왓슨에서 서비스의 내용을 바꾸길 원하지 않으면 헤더에
		  그 내용을 명시해 주어야 한다.
	*/
	...
	public void executeService() {
		String text = "Hello! My name is Allison, Nice to meet you.";
		
		InputStream stream = 
			service.synthesize(text, Voice.EN_ALLISON, AudioFormat.WAV).execute();
	...
	public static void main(String[] args) {
		TestToSpeechTest test  = new TestToSpeechTest();
		test.setService();
		test.setHeader();
		//test.getVoice();
		test.executeService();
