...
/*
	UDP방식 : 비 연결지향, 비 신뢰성, 데이터가 순서대로 도착한다는 보장이 없다.
			하지만 TCP방식보다 빠르게 전돨된다.
			
	DatagramSocket, DatagramPacket을 사용한다.
	- DatagramSocket => 데이터 송수신과 관련된 클래스 (우체부)
	- DatagramPacket => 주고 받을 데이터와 관련된 클래스 (소포)
			==> 수신을 위한 생성자와 송신을 위한 생성자, 두가지를 제공
			
	- TCP의 경우는 스트림을 이용하여 데이터를 송수신 하지만 
	  UDP의 경우 데이터그램을 이용하여 데이터를 송수신 한다.
*/
