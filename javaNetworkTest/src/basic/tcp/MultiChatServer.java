...
public class MultiChatServer {
	...
	public void serverStart() {
		...
		try {
			...
			if(server!=null)try { server.close(); } catch(IOException ee) {}
		}
	}
	
	// '대화방Map'에 있는 전체 사용자에게 메시지를 보내는 메서드
	public void sendToAll(String msg) {
		// 대화방 Map에 저장된 key값(사용자이름)구하기
		try {
	...
	class ServerReceiver extends Thread{
		...
		public ServerReceiver(Socket socket) {
			...
			try {
				...
				// 클라이언트로 메시지를 송신하기 위한 OutputStream객체
		...
		public void run() {
			...
			try {
				while(true) {
					...
					if (clients.containsKey(name)) { 	// 대화명이 중복딜 때
						dos.writeUTF("이름 중복");
					}else {
						dos.writeUTF("OK");
						break;
				...
				// 대화명을 현재 접속된 전체 사용자에게 알림메시지를 보낸다.
