...
//			new Member("홍길서", "gilseo", 43, "3333-3333", "광주"),
//			new Member("홍길남", "gilnam", 23, "2222-2222", "인천"),
//			new Member("홍길북", "gilbook", 53, "7777-7777", "서울")
//		);
		List<Member> memList = new ArrayList<>();
		memList.add(new Member("홍길동", "gildong", 33, "1234-5678", "대전"));
		memList.add(new Member("홍길서", "gilseo", 43, "3333-3333", "광주"));
		memList.add(new Member("홍길남", "gilnam", 23, "2222-2222", "인천"));
		memList.add(new Member("홍길북", "gilbook", 53, "7777-7777", "서울"));
		...
		*/
		...
		TableColumn<Member, String> engNameCol = new TableColumn<>("영문이름");
...
//		ageCol.setPrefWidth(150);
		...
		btnAdd.setOnAction(e->{
			...
			if (tfEngName.getText().isEmpty() || tfAge.getText().isEmpty() || 
			...
			data.add(new Member(tfKorName.getText(), tfEngName.getText(), Integer.parseInt(tfAge.getText()), tfTel.getText(), tfAddr.getText()));
			
			AlertUtil.warning("확인", "작업성공", tfKorName.getText()+"씨의 정보가 추가되었습니다.");
		...
		btnEdit.setOnAction(e->{
			...
			if (table.getSelectionModel().isEmpty()) {
				AlertUtil.warning("경고", "자료선택오류", "수정할 데이터가 없습니다.");
				return;
			}
			...
			data.set(index, new Member(tfKorName.getText(), tfEngName.getText(), 
					Integer.parseInt(tfAge.getText()), tfTel.getText(), 
					tfAddr.getText()));
			
			AlertUtil.warning("확인", "수정작업성공", tfKorName.getText()+"씨의 정보가 수정되었습니다.");
		});
...
//			String name = data.get(index).getKorName();
		...
		table.setOnMouseClicked(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				...
				if (m!=null) {
					...
					tfAge.setText(String.valueOf(m.getAge())); // "" + m.getAge()
		...
		// disable 속성 ==> 해당 컨트롤을 활성화 또는 비활성화 시킨다.
		//		btnAdd.setDisable(true); // 비활성화 --> 비활성화된 컨트롤은 사용불가 사용할수 없다.
