...
public class JavaFxProgramLayout extends Application {
	/*
		javafx에서 사용되는 객체 분류
	1. 컨테이너 객체들 ==> 다른 컨테이너나 객체나 컨트롤 객체를 가지고 있는 객체
	
	2. 컨트롤 객체 ==> 사용자가 직접 사용하는 객체 (예 : 버튼, 체크박스, 리스트박스 등)
	
		프로그래밍 작업 순서
	컨테이너객체 작성 => 컨트롤객체들 작성 ==>  컨트롤객체들을 컨테이너 객체에 배치한다.
	==> 루트 컨테이너 객체를 Scene객체에 추가 ==> Scene객체를 Stage추가
	==> show()메서드 호출
	...
	*/	
	...
	public void start(Stage primaryStage) {
		...
		root.setAlignment(Pos.CENTER); // 배치될 컨트롤들을 가운데 정렬
		root.setSpacing(20); 	// 컨트롤 객체들 간의 간격
		
		// 안쪽여백 ==> Insets객체를 이용해서 설정한다.
